...
P(T_+ \;| A) &= 0.99 \\
P(T_- \;| A) &= 0.05 \\
P\left(T_+ \;| A^C\right) &= 0.01 \\
P\left(T_- \;| A^C\right) &= 0.95 
\end{align*}
$$ As an aside, $P(T_+\;| A)$ is the true positive rate, also known as the *sensitivity* of the test; $P(T_- \;| A^C)$ is the true negative rate, also known as the *specificity*. Note that although in this example, $P(T_+ \;| A) + P(T_+ \;| A^C) = 1$, and likewise for $T_-$, but this need not be the case.
...
P(A \;| T_-) 
&= \frac{P(T_- \;| A) P(A)}{P(T_- \;| A) P(A) + P\left(T_- \;| A^C\right) P\left(A^C\right)}  \\
&= \frac{(0.05)(0.005)}{(0.05)(0.005) + (0.95)(0.995)} \\
&\approx 0.00026
\end{align*}
$$

This usually comes as a surprise, given that the test appears to be fairly accurate, but somewhat prone to false negatives. What happens if the condition is more prevalent, say, present in $5\%$ of the population? In this scenario:

$$
\begin{align*}
P(A \;| T_+) 
&= \frac{(0.99)(0.05)}{(0.99)(0.05) + (0.01)(0.95)} 
\approx 0.84 \\
P(A \;| T_-) 
&= \frac{(0.05)(0.05)}{(0.05)(0.05) + (0.95)(0.95)}
\approx 0.0028
\end{align*}
$$
